            ...
            <div className="relative px-4">
                ...
                <div className="mt-6 flex items-center justify-between px-4">
                    <div className="flex gap-2">
                        <Button variant="outline" size="icon" className="h-8 w-8 rounded-full border-border/50 bg-background/80 hover:bg-background" onClick={() => emblaApi?.scrollPrev()}>
                            <ChevronLeft className="h-4 w-4 text-foreground/80" />
                            <span className="sr-only">Previous slide</span>
                        </Button>
                        <Button variant="outline" size="icon" className="h-8 w-8 rounded-full border-border/50 bg-background/80 hover:bg-background" onClick={() => emblaApi?.scrollNext()}>
                    ...
                    <div className="flex gap-2">
                        {scrollSnaps.map((_, index) => (
                            <button key={index} type="button" className={`h-3 w-3 rounded-full border border-border p-0 ${index === selectedIndex ? "bg-foreground" : "bg-background"}`} onClick={() => scrollTo(index)} aria-label={`Go to slide ${index + 1}`} />
                        ))}
                    </div>
                </div>
            </div>
