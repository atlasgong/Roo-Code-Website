    ...
    return (
        <>
            <div className="relative inline-flex" ref={buttonRef}>
                <canvas ref={canvasRef} className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2" style={{ pointerEvents: "none" }} />
                <Link
                    href="#"
                    onClick={(e) => {
                        e.preventDefault();
            ...
            <AnimatePresence>
                {isOpen && (
                    <>
                        <motion.div className="fixed inset-0 z-40 bg-black/80 backdrop-blur-sm" initial={{ opacity: 0, backdropFilter: "blur(0px)" }} animate={{ opacity: 1, backdropFilter: "blur(8px)" }} exit={{ opacity: 0, backdropFilter: "blur(0px)" }} transition={{ duration: 0.2 }} />
                            ...
                            <div className="flex min-h-full items-center justify-center p-4">
                                ...
                                >
                                    <div className="flex items-center justify-between gap-4">
                                        <h2 className="text-xl font-bold sm:text-2xl">What&apos;s New in Roo Code {version}</h2>
                                        <button onClick={() => setIsOpen(false)} className="flex-shrink-0 rounded-full p-1.5 text-gray-400 hover:bg-gray-800 hover:text-white">
                                            <X className="h-5 w-5" />
                                        </button>
                                    </div>

                                    <div className="mt-4 space-y-4 sm:mt-6 sm:space-y-6">
                                        <Feature icon={Code2} color="blue" title="AI-Powered Code Generation" description="Generate high-quality code snippets and entire components with our new AI assistant. Trained on millions of code repositories to understand your project context." />
                                        <Feature icon={Users2} color="purple" title="Real-time Collaboration" description="Work together with your team in real-time with our new collaborative editing features. See changes as they happen and resolve conflicts automatically." />
                                        <Feature icon={Zap} color="green" title="Performance Optimizations" description="We've completely rewritten our core engine for blazing fast performance. Experience up to 10x faster build times and smoother development workflow." />
                                    </div>
                                </motion.div>
                            </div>
                        </div>
